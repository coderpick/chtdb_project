    ...
    <form action="{{ route('student.career.update') }}" method="POST">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select name="status" class="dash-form-control @error('status') is-invalid @enderror" id="careerStatus" onchange="toggleCareerFields()" required>
                    ...
                    <option value="job_and_freelance" {{ old('status', $user->career->status ?? '') == 'job_and_freelance' ? 'selected' : '' }}>চাকরি ও ফ্রিল্যান্সিং দুটোই</option>
                    <option value="seeking" {{ old('status', $user->career->status ?? '') == 'seeking' ? 'selected' : '' }}>চাকরি খুঁজছি</option>
                    <option value="higher_education" {{ old('status', $user->career->status ?? '') == 'higher_education' ? 'selected' : '' }}>উচ্চশিক্ষায় আছি</option>
                </select>
            ...
            <div class="col-12" id="jobFields" style="display:none;">
                <div class="row g-3" style="background:#f8faf9;border-radius:14px;padding:20px;margin-top:4px;">
                    <div class="col-12">
                        <h6 style="font-weight:700;color:var(--primary);"><i class="bi bi-building me-1"></i> চাকরির তথ্য</h6>
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">প্রাতিষ্ঠানের নাম</label>
                        <input type="text" name="company" class="dash-form-control @error('company') is-invalid @enderror" value="{{ old('company', $user->career->company ?? '') }}" placeholder="কোম্পানি/প্রতিষ্ঠানের নাম">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="join_date" id="join-date-picker" class="dash-form-control @error('join_date') is-invalid @enderror" value="{{ old('join_date', isset($user->career->join_date) ? \Carbon\Carbon::parse($user->career->join_date)->format('Y-m-d') : '') }}" placeholder="YYYY-MM-DD">
            ...
            <div class="col-12" id="freelanceFields" style="display:none;">
                <div class="row g-3" style="background:#f0f4ff;border-radius:14px;padding:20px;margin-top:4px;">
                    <div class="col-12">
                        <h6 style="font-weight:700;color:#0d6efd;"><i class="bi bi-globe me-1"></i> ফ্রিল্যান্সিং তথ্য</h6>
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">ফ্রিল্যান্সিং প্ল্যাটফর্ম</label>
                        <input type="text" name="platform" class="dash-form-control @error('platform') is-invalid @enderror" value="{{ old('platform', $user->career->platform ?? '') }}" placeholder="Upwork, Fiverr, Freelancer ইত্যাদি">
                        @error('platform')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">প্রোফাইল লিংক</label>
                        <input type="url" name="profile_link" class="dash-form-control @error('profile_link') is-invalid @enderror" value="{{ old('profile_link', $user->career->profile_link ?? '') }}" placeholder="https://www.upwork.com/freelancers/...">
                        @error('profile_link')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">মোট সম্পন্ন প্রজেক্ট</label>
                        <input type="number" name="completed_projects" class="dash-form-control @error('completed_projects') is-invalid @enderror" value="{{ old('completed_projects', $user->career->completed_projects ?? '') }}" placeholder="যেমন: 25">
                        @error('completed_projects')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">ক্লায়েন্ট রেটিং</label>
                        <input type="text" name="rating" class="dash-form-control @error('rating') is-invalid @enderror" value="{{ old('rating', $user->career->rating ?? '') }}" placeholder="যেমন: 4.9/5.0">
            ...
            <div class="col-12" id="entrepreneurFields" style="display:none;">
                <div class="row g-3" style="background:#fef9e7;border-radius:14px;padding:20px;margin-top:4px;">
                    <div class="col-12">
                        <h6 style="font-weight:700;color:var(--secondary);"><i class="bi bi-shop me-1"></i> উদ্যোক্তা তথ্য</h6>
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">ব্যবসা/প্রতিষ্ঠানের নাম</label>
                        <input type="text" name="business_name" class="dash-form-control @error('business_name') is-invalid @enderror" value="{{ old('business_name', $user->career->business_name ?? '') }}" placeholder="আপনার ব্যবসার নাম">
                        @error('business_name')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">ব্যবসার ধরন</label>
                        <input type="text" name="business_type" class="dash-form-control @error('business_type') is-invalid @enderror" value="{{ old('business_type', $user->career->business_type ?? '') }}" placeholder="ডিজিটাল এজেন্সি / ই-কমার্স ইত্যাদি">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="employees" class="dash-form-control @error('employees') is-invalid @enderror" value="{{ old('employees', $user->career->employees ?? '') }}" placeholder="যেমন: 5">
                        @error('employees')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="dash-form-label">ওয়েবসাইট (যদি থাকে)</label>
                        <input type="url" name="business_website" class="dash-form-control @error('business_website') is-invalid @enderror" value="{{ old('business_website', $user->career->business_website ?? '') }}" placeholder="https://example.com">
                        @error('business_website')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                </div>
            </div>
