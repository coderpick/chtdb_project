...
<div class="card border-0 shadow-sm" style="border-radius: 15px;">
    <div class="card-body p-4">
        <form action="{{ route('admin.success-stories.update', $successStory->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select name="user_id" class="form-select select2 @error('user_id') is-invalid @enderror" required>
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('user_id', $successStory->user_id) == $user->id ? 'selected' : '' }}>
            ...
            <div class="row mb-3">
                    ...
                    <select name="district_id" id="district_id"
                        ...
                        @foreach ($districts as $district)
                            <option value="{{ $district->id }}"
                                {{ old('district_id', $successStory->district_id) == $district->id ? 'selected' : '' }}>
                                {{ $district->bn_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="upazila_id" id="upazila_id" class="form-select @error('upazila_id') is-invalid @enderror">
                        <option value="">Select Upazila</option>
                        @foreach ($upazilas as $upazila)
                            <option value="{{ $upazila->id }}"
                                {{ old('upazila_id', $successStory->upazila_id) == $upazila->id ? 'selected' : '' }}>
                                {{ $upazila->bn_name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                <textarea name="story_text" rows="5" class="form-control @error('story_text') is-invalid @enderror"
            ...
            <div class="mb-4">
                ...
                <div class="btn-group status-radio-group w-100" role="group">
                    <input type="radio" class="btn-check" name="status" id="statusPending" value="pending"
                        {{ old('status', $successStory->status) == 'pending' ? 'checked' : '' }} required>
                    ...
                    <input type="radio" class="btn-check" name="status" id="statusApproved" value="approved"
                        {{ old('status', $successStory->status) == 'approved' ? 'checked' : '' }}>
                    ...
                    <input type="radio" class="btn-check" name="status" id="statusRejected" value="rejected"
                        {{ old('status', $successStory->status) == 'rejected' ? 'checked' : '' }}>
