    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <a href="{{ route('admin.centers.create') }}" class="btn btn-sm btn-success"><i class="bi bi-plus"></i> Add
                    Center</a>
            </div>
                    ...
                    <tbody>
                        @forelse($centers as $center)
                            <tr>
                                <td>{{ $center->id }}</td>
                                <td>{{ $center->name }}</td>
                                <td>{{ ucfirst($center->district->bn_name) }}</td>
                                <td>{{ $center->phone }}</td>
                                <td>{{ $center->email }}</td>
                                <td>{{ $center->total_trainee }}</td>
                                <td>
                                    @if ($center->is_active)
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center">No centers found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
